      ...
      <div className="shrink-0 flex items-center justify-between px-4 h-16 bg-sky-500 z-50 shadow-md">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <span className="text-[10px] font-bold text-sky-100 uppercase tracking-wider">Messages</span>
            <h2 className="text-sm font-black text-white tracking-tight uppercase">@{auth.currentUser?.displayName || 'My Chats'}</h2>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar pb-20">
        ...
        <div className="px-4 my-4">
          <div className="relative">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-zinc-100 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-all"
            />
          </div>
        </div>
        ...
        <div className="mt-2">
          <div className="px-4 flex justify-between items-center mb-4">
            <span className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Recent Chats</span>
            <span className="text-blue-500 text-xs font-bold uppercase tracking-wider cursor-pointer hover:underline">Requests</span>
          </div>
          
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              <div className="w-8 h-8 border-4 border-blue-500/20 border-t-blue-500 rounded-full animate-spin" />
              <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Loading Chats...</p>
            </div>
            ...
            <div className="divide-y divide-zinc-50">
              {filteredConversations.map(chat => (
                <Link 
                  ...
                  key={chat.id} 
                  className="flex items-center gap-4 px-4 py-4 hover:bg-zinc-50 transition-all active:scale-[0.98]"
                >
                  <div className="relative">
                    <img 
                      src={chat.avatar} 
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-baseline mb-0.5">
                      <h3 className={`text-[15px] truncate ${chat.unread ? 'font-black text-zinc-900' : 'font-bold text-zinc-800'}`}>
                        {chat.user}
                      </h3>
                      <span className={`text-[10px] whitespace-nowrap ${chat.unread ? 'text-blue-600 font-bold' : 'text-zinc-400'}`}>
                        {chat.time}
                      </span>
                    </div>
                    <p className={`text-xs truncate ${chat.unread ? 'text-zinc-900 font-bold' : 'text-zinc-500'}`}>
                      {chat.lastMsg}
                    </p>
                  </div>
                  {chat.unread && (
                    <div className="flex flex-col items-center gap-1">
                      <div className="min-w-[18px] h-[18px] px-1 bg-blue-600 rounded-full flex items-center justify-center shadow-lg shadow-blue-200">
            ...
            <div className="flex flex-col items-center justify-center py-20 px-10 text-center gap-4">
              ...
              <button 
                ...
                className="mt-2 bg-blue-600 text-white px-6 py-2.5 rounded-xl text-xs font-bold shadow-lg shadow-blue-100 hover:bg-blue-700 transition-all"
              >
                Find Friends
              </button>
            </div>
        ...
        <div className="py-10 flex flex-col items-center gap-1 opacity-50">
          <span className="text-zinc-400 text-[10px] font-bold tracking-widest uppercase">GxChat India Messages</span>
          <span className="text-zinc-400 text-[8px] uppercase tracking-tighter">End-to-end encrypted</span>
        </div>
      </div>
    </div>
  );
